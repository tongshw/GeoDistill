...
def calculate_entropy(similarity_map):
    ...
    hist, bin_edges = np.histogram(flattened_map, bins=100, range=(0, 1), density=True)
...
def validate_distillation(args, model, val_loader, criterion, device, epoch=-1, vis=False, name=None):
    model.eval()
    total_loss = 0
    all_errors = []
    ...
    with torch.no_grad():
        ...
        for i_batch, data_blob in enumerate(pbar):
            ...
            sat_feat_dict, sat_conf_dict, g2s1_feat_dict, g2s1_conf_dict, g2s2_feat_dict,\
                g2s2_conf_dict, mask1_dict, mask2_dict = model(sat, resized_pano, ones2, pano1, ones1, meter_per_pixel)
            ...
            for i in range(B):
                ...
                if entropy_t > entropy_s:
                    ...
                    if args.save_visualization:
                        save_path1 = f"./vis/distillation/{args.model_name}/test/teacher/{city[i]}_{sat_gps[i].cpu().numpy()}.png"
                        save_path2 = f"./vis/distillation/{args.model_name}/test/student/{city[i]}_{sat_gps[i].cpu().numpy()}.png"

                    vis_corr(corr_t[i], sat[i], resized_pano[i], gt_points[i], None, save_path1, temp=1)
                    vis_corr(corr_s[i], sat[i], pano1[i], gt_points[i], None, save_path2, temp=1)
...
def test(args):
    ...
    mean_dis_t, mean_dis_s, median_dis_t, median_dis_s = validate_distillation(args, model, test_loader, criterion, device, name="student")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default="cross-eval_uncertainty0.06-fov240", help="none")
